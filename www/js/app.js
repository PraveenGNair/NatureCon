...
.constant('FORECASTIO_KEY', '15db1c8368a6261aea5b7c8e19dc5451')
...
.controller('WeatherCtrl', function($scope, $timeout, $rootScope, Weather, Geo, Flickr, $ionicModal, $ionicPlatform) {
  ...
  $scope.showSettings = function() {
    if(!$scope.settingsModal||!$scope.settingsModal._isShown) {
  ...
  this.getCurrent = function(lat, lng, locString) {
    Weather.getAtLocation(lat, lng).then(function(resp) {
      ...
      if(resp.response && resp.response.error) {
        alert('This Wunderground API Key has exceeded the free limit. Please use Wunderground key');
        return;
      }
      */
